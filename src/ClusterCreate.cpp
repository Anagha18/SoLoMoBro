...
typedef struct client_ret_details client_ret_details;
...
typedef struct client_info_cs client_info_cs;
...
void *connection_handler(void *socket_desc){
   ...
   //this is the part where yo replicate the next_servers table and also the so files(if this client has replicate var set to true)
    if(replicate){
        int prio = replicate-1;
        write(sock, "file", 5);
        write(sock, &prio, sizeof(int));
        vector<string> files;
...
int replicate_state(int clientSocket, map<int, client_info_cs>* next_servers, int* prio){
    //tab begin
    int file_nos = 0;
    char temp[25];
    read(clientSocket, temp, 5);
    if(strcmp(temp,"file") == 0){
        read(clientSocket, prio, sizeof(int));
        ...
        if(mkdir("../op_files",0777)==-1){
            printf("The op_files directory could not be created!.\n");
        }
    ...
    for(int i = 0; i<client_nos; i++){
        ...
        next_servers->insert(pair<int, client_info_cs> (order, c));
    }
...
void handle_server_failure(int prio, int port, map<int,client_info_cs>* next_servers, client_ret_details* crd){
    if(prio==0){
        crd->server = 1;
        crd->port = port+1;
    }
        ...
        crd->server = 0;
        strcpy(crd->serv_addr, (*next_servers)[0].ipAddr );
        crd->serv_port = port+1;
    }
}


client_ret_details* set_up_client(char* addr, int port){
    //tab begin
	int clientSocket;
    int ret;
    int prio = -1;// -1 means it still has not been chosen so as to be the next server!
	struct sockaddr_in serverAddr;
	struct sockaddr_in clientAddr;
	char buffer[BUFFER_LEN];
	map<int, client_info_cs> next_servers;
    client_ret_details* crd = (client_ret_details*)malloc(sizeof(client_ret_details));
    ...
    replicate_state(clientSocket, &next_servers, &prio);
	while (1){
        ...
        if(t==-1 || t==0){ //it actually return 0
            ...
            // exit(1);
            handle_server_failure(prio, port, &next_servers, crd);
            return crd;
        }
